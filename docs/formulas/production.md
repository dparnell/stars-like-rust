...
- The build queue itself: item costs come from the components table, which is
  not decoded. `build.c` in the reconstructed sources is mostly stubs, so this
  will have to be read from our binary.
- Auto-build queue items (mines, factories, defenses) are the tractable first
  step, since their costs are race attributes rather than component data.
