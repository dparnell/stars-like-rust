...
| `hull-schematics.json` | Where every hull's slots and cargo space sit on the ship designer's schematic | Read out of `rghuldef` (`1008:29f0`) and `rghuldefSB` (`1008:4872`) in `binary/stars.2.7j.exe` — `HULDEF.rgbrc` and `HULDEF.wrcCargo` |
