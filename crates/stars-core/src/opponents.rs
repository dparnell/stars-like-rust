...
//!
//! Each entry's `grbitAttr` in the reconstructed table sets bits outside the
//! fourteen lesser racial traits a player may choose: bit 29
//! (`ibitRaceTech3`) and bit 31 (`ibitRaceCheapFact`) on the harder entries.
//! A race **record on disk** carries only sixteen bits, so those two can never
//! reach us through a file, and [`crate::Race`] models the fourteen real
//! traits alone.
//!
//! They are masked off here rather than carried, because the turn-0 fixture
//! settles it: its two computer players are `Turindrones, Standard` byte for
//! byte (same economy, same habitability bands, same growth rate), whose entry
//! sets bit 31. Their homeworlds were stocked with the leftover advantage
//! points of a **50-point** race, and 50 is what
//! [`crate::advantage_points`] returns for that race only with bit 31 clear —
//! Cheap Factories would cost it 58 points and leave 14. The raw value is kept
//! in [`Opponent::raw_flags`] so the discrepancy stays visible.
...
pub struct Opponent {
    ...
    /// The race it plays.
    pub race: Race,
    /// The entry's `grbitAttr` exactly as the table holds it, including the
    /// bits above the fourteen selectable traits (see the module docs).
    pub raw_flags: u32,
}
